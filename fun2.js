...
function setValue(){
        ...
        if(score==0){
            questionId.textContent=`{ ${score}/${questions.length} } You in a Wrong Path`
        }
        else if(score<5){
            questionId.textContent=`{ ${score}/${questions.length} } Focus on your Study `
        }
            ...
            questionId.textContent=`{ ${score}/${questions.length} }  Parampara, Pratishtha, Anushasan. `
        }
...
prevButton.addEventListener('click', function() {
    ...
    if (currentQuestionIndex >= 0) {
        showAllQuestions(currentQuestionIndex);
        // displayPreviousAnswer(currentQuestionIndex); // Highlighting user's previously selected answer
        nextButton.style.display = "block"; 
    } else {
